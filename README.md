...
一个 AI 原生的知识探索游戏：模型会为陌生领域实时编写可操作的 HTML / CSS / SVG / Canvas 世界，让好奇心在星图中连成路径。
...
点、线、面、体只描述知识如何生长，并不限制一个知识点如何呈现。每次观测都可以由模型生成完全不同的浏览器原生交互程序：轨道实验、生态模拟、城市流量、声音波形、机械结构或未来模型能够写出的其他形式。生成代码在无网络权限的隔离沙箱中运行；固定图表只作为失败兜底。
...
- vinext / Vite
- DashScope Qwen
- 模型实时生成的 HTML、CSS、SVG、Canvas 交互世界
- 沙箱隔离、CSP 与受控消息桥
- LocalStorage 个人星图存档
